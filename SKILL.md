...
- **独立使用**：在对话中直接调用 `$frontend-dev-guide` 或提及触发词，即可独立执行前端开发工作流。每次执行会自动写入 `.skill-memory/frontend-dev-guide/` 形成记忆积累。
- **被指挥官调用**：如果你已安装 [team-orchestrator](https://github.com/genapohub/team-orchestrator) 调度指挥官，本技能可作为虚拟研发团队的一员，被自动路由调度参与全流程项目协作。被调用时会从 `.skill-memory/frontend-dev-guide/` 加载历史记忆，并与指挥官同步项目上下文，实现无缝角色切换。
...
1. 检查项目目录下是否存在 `.skill-memory/frontend-dev-guide/` 目录，如不存在则创建
2. 读取 `.skill-memory/frontend-dev-guide/MEMORY.md`（如果存在）——了解长期技术决策、架构约定、组件规范等
3. 读取 `.skill-memory/frontend-dev-guide/YYYY-MM-DD.md`（今日日志，如果存在）——了解近期已完成的开发任务

加载记忆后，在回复中简要提及相关的历史上下文（如"根据之前的架构决策，该项目使用 React + Zustand..."），让用户感受到连贯性。
...
1. **写入今日日志** — 向 `.skill-memory/frontend-dev-guide/YYYY-MM-DD.md` **追加**（不存在则先创建）：
   - 场景类型（场景X）
   - 开发的模块/功能名称
   - 产出的技术文档清单
   - 关键架构决策（如技术栈选择、组件拆分方案、性能优化策略等）
   - 待后续跟进的事项

2. **更新长期记忆** — 如果产生了**可复用的技术规范或决策**（如技术栈选型结论、组件设计模式、性能优化策略、测试规范等），同步更新 `.skill-memory/frontend-dev-guide/MEMORY.md`：
   - 按主题分类（技术栈 / 架构设计 / 组件规范 / 性能策略 / 测试约定）
   - 保持简洁，每条 1-2 行

**只需记录实质性工作**（产出了架构方案/组件设计/性能方案等）。纯问答、Bug 单行修复不需记录。

3. **同步到仓库** — 可将 `.skill-memory/frontend-dev-guide/` 下的变更提交到 Git，实现团队共享记忆。

**记忆目录结构：**
```
项目目录/.skill-memory/frontend-dev-guide/
├── 2026-07-22.md
└── MEMORY.md
```
